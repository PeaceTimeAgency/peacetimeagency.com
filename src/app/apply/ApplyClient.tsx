...
function ApplicationForm({ initialCreators = [] }: { initialCreators: any[] }) {
...
export default function ApplyClient({ initialCreators = [] }: { initialCreators: any[] }) {
  return (
    <Suspense fallback={<div className="min-h-screen bg-background pt-24 pb-12 flex items-center justify-center text-primary font-black uppercase tracking-widest text-sm mt-[68px]">Loading Form...</div>}>
      <ApplicationForm initialCreators={initialCreators} />
    </Suspense>
  );
}
